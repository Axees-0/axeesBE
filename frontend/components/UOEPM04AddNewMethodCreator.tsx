...
const styles = StyleSheet.create({
  ...
  webContainer: {
    ...
    paddingHorizontal: Platform.select({ default: '5%', web: '15%' }),
    marginTop: Platform.select({ default: 0, web: "2.5%" }),
    width: "100%",
  },
  inner:{ flex:1 },
  ...
  header:{ flexDirection:'row',justifyContent:'space-between',alignItems:'center',
           ...
           paddingTop: Platform.OS==='ios'?20:24,paddingBottom:20 },
  headerTitle:{ fontSize: Platform.select({ default: 20, web: 24 }),
                fontWeight:'600',color:'#000' },

  content:{ flex:1,paddingHorizontal: Platform.select({ default: 16, web: 32 }) },
  section:{ marginBottom: Platform.select({ default: 24, web: 40 }) },
  sectionTitle:{ fontSize: Platform.select({ default: 20, web: 24 }),
                 fontWeight:'500',color:'#000',
                 marginBottom: Platform.select({ default: 16, web: 24 }) },

  inputGroup:{ marginBottom:16 },
  row:{ flexDirection: Platform.select({ default: 'column', web: 'row' }),
        gap: Platform.select({ default: 0, web: 16 }) },
  label:{ fontSize: Platform.select({ default: 13, web: 14 }),
          color:'#6C6C6C',marginBottom:6 },
  textInput:{ borderWidth:1,borderColor:'#E2D0FB',borderRadius:8,
              paddingHorizontal: Platform.select({ default: 12, web: 16 }),
              height: Platform.select({ default: 44, web: 48 }),
              fontSize: Platform.select({ default: 14, web: 16 }),
              color:'#000',
              marginBottom: Platform.select({ default: 16, web: 0 }) },

  toggle:{ fontSize: Platform.select({ default: 14, web: 16 }),
           fontWeight:'500',color:'#430B92',textDecorationLine:'underline',
           marginBottom:10,
           textAlign: Platform.select({ default: 'left', web: 'left' }) },

  cta:{ backgroundColor:'#430B92',borderRadius:8,
        height: Platform.select({ default: 50, web: 58 }),
        justifyContent:'center',alignItems:'center',
        margin: Platform.select({ default: 16, web: 32 }),
        marginHorizontal: Platform.select({ default: 16, web: 32 }) },
  ctaText:{ color:'#fff',fontSize: Platform.select({ default: 16, web: 18 }),
            fontWeight:'500' },
});
