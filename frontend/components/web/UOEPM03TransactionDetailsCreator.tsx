...
export default function TransactionDetailsCreator() {
  ...
  const fetchWithDrawalById = async (withdrawalId: string): Promise<WithDrawal> => {
    const response = await axios.get(
      `${process.env.EXPO_PUBLIC_BACKEND_URL}/api/payments/withdrawal/${withdrawalId}`, // Pass withdrawalId in the URL path
      {
    ...
    const fetchPayoutById = async (payoutId: string): Promise<Payout> => {
      const response = await axios.get(
        `${process.env.EXPO_PUBLIC_BACKEND_URL}/api/payments/getPayoutById/${payoutId}`, // Pass payoutId in the URL path
        {
          params: { userId: user?._id } // Optional: if you need to send the userId as a query parameter for authorization
        }
      );
  ...
  const downloadReceiptMarketer = async () => {
    if (!payout) return;
  ...
  const downloadReceiptCreator = async () => {
    ...
    <html>
      ...
      <body>
        <h1>Transaction Receipt</h1>
        <table>
          <tr><th>Deal</th><td>${withdrawal.status}</td></tr>
          <tr><th>Transaction ID</th><td>${withdrawal.transactionId ?? "N/A"}</td></tr>
          <tr><th>Payment Date</th><td>${withdrawal.completedAt}</td></tr>
          <tr><th>Payment Method</th><td>${withdrawal.paymentMethod ?? "N/A"}</td></tr>
          <tr><th>Amount</th><td class="amount">$${withdrawal.amount.toFixed(2)}</td></tr>
        </table>
      </body>
    </html>
  `;
    ...
    try {
      const { uri } = await Print.printToFileAsync({ html });
      const pdfName = `receipt_${withdrawal._id}.pdf`;
      const dest = FileSystem.cacheDirectory + pdfName;
      await FileSystem.moveAsync({ from: uri, to: dest });
...
const {
  ...
  queryFn: () => fetchWithDrawalById(id as string),
...
const {
  ...
  enabled: !!id && isMarketer,
});
if ((isCreator && isWithdrawalLoading) || (isMarketer && isPayoutLoading)) {
  return <Text>Loading transaction...</Text>;
}

if ((isCreator && (withdrawalError || !withdrawal)) || (isMarketer && (payoutError || !payout))) {
  ...
  if (isCreator && withdrawal?.createdAt && !isNaN(Date.parse(withdrawal.createdAt))) {
    paymentDate = format(new Date(withdrawal.createdAt), "dd MMM yyyy");
  } else if (isMarketer && payout?.completedAt && !isNaN(Date.parse(payout.completedAt))) {
    paymentDate = format(new Date(payout.completedAt), "dd MMM yyyy");
  }
  

  return (
    <>
    {/* <Navbar pageTitle="Payout Details"/> */}
    <SafeAreaView style={styles.container}>      
      <StatusBar style="auto" />
      {isCreator && (<View style={[styles.flexContainer,isWeb && isWideScreen && styles.webContainer]}>
      ...
      <ScrollView style={styles.content}>
        <Text style={styles.transactionName}>
          {withdrawal.status || "Transaction"}
        </Text>
        <View style={styles.detailsList}>
          {/* <View style={styles.detailItem}>
            <Text style={styles.detailLabel}>Deal No</Text>
            <Text style={styles.detailValue}>
              {withdrawal.deal ? withdrawal.deal.dealNumber : "N/A"}
            </Text>
          </View> */}
          <View style={styles.detailItem}>
          ...
          <View style={styles.detailItem}>
            ...
            <Text style={styles.detailValue}>
              {withdrawal.paymentMethod || "Credit Card"}
            </Text>
          </View>
          <View style={styles.detailItem}>
            <Text style={styles.detailLabel}>Depositing Account</Text>
            <Text style={styles.detailValue}>
              {withdrawal.image ? "Image Available" : "N/A"}
            </Text>
          </View>
          <View style={styles.detailItem}>
            <Text style={styles.detailLabel}>Amount</Text>
            <Text style={[styles.detailValue, styles.amountValue]}>
              {withdrawal.amount != null && !isNaN(withdrawal.amount)
      ...
      <TouchableOpacity style={styles.downloadButton} onPress={downloadReceiptCreator}>
        <Text style={styles.downloadButtonText}>Download Receipt</Text>
      </TouchableOpacity>
      </View>)}

      {isMarketer && (<View style={[styles.flexContainer,isWeb && isWideScreen && styles.webContainer]}> 
            {/* <Navbar pageTitle="Payout Details"/> */}
      <ScrollView style={styles.content}>
        <Text style={styles.transactionName}>
      ...
      <TouchableOpacity style={styles.downloadButton} onPress={downloadReceiptMarketer}>
        <Text style={styles.downloadButtonText}>Download Receipt</Text>
      </TouchableOpacity>
      </View>)}
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  webContainer: {
    width: "100%",
    paddingHorizontal: "5%", // 5% margin on both sides
  },
  ...
  downloadButton: {
    ...
    alignItems: "center",
    marginVertical: 20,
    width: "30%",           // Limit width to 40% of screen
    alignSelf: "center",    // Center it horizontally
    minWidth: 160,          // Optional: ensure it's usable on small screens
    paddingHorizontal: 16,  // Optional: add internal padding for content
  },
  flexContainer: {
    flex: 1,
    justifyContent: "space-between",
    paddingBottom: 20, // for breathing room at bottom
  },
  
  scrollContent: {
    padding: 20,
    paddingBottom: 40, // avoid overlap with button
  },
