...
import { FontFamily,Border } from "@/GlobalStyles";
...
export default function MarketerOfferHistoryList() {
  ...
  const possibleStatuses = useMemo(() => {
    ...
    // If user.userType === "Marketer", use "Viewed by Creator"
    // If user.userType === "Creator", use "Viewed by Marketer"
          ...
          if (draft.draftType === "Counter") {
            router.push({
              ...
              params: {
                ...
                type: "draft",
              }, // Pass draftId instead of offerId
            });
          } else {
            router.push({
  ...
  // Combine ONLY drafts and apply filtering & sorting
  const combinedItems = [...(data?.newDrafts || [])]
    .map((draft) => ({ ...draft, isDraft: true }))
    .filter((item) => {
      if (!filterStatus) return true;
      return item.isDraft ? filterStatus === "Draft" : false;
    })
    .sort((a, b) => {
