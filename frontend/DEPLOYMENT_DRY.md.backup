...
- **Site IDs and URLs** - No more hardcoded 'polite-ganache-3a4e1b' scattered across files
...
```bash
npm run deploy              # Production with build
npm run deploy:quick        # Production without build  
...
const siteId = 'polite-ganache-3a4e1b';
...
3. **Consistent Interface** - One deployment command with environment parameter
4. **Easier Testing** - Centralized config makes mocking easier
