...
//--- create manager api instance
   ...
   m_manager->LogsMode(LOG_MODE_ENABLED);
//---
   ...
   if(m_plugins    !=NULL)
     {
      for(int i=0;i<m_total;i++)
         if(m_plugins[i].params!=NULL)
           {
            m_manager->MemFree(m_plugins[i].params);
            m_plugins[i].params=NULL;
           }
...
//--- запрашиваем
   m_performance=m_manager->PerformanceRequest(from,&m_total);
   DisplayPerformance();
//---
  }
//+------------------------------------------------------------------+
   ...
   switch(m_display_type)
     {
      ...
      case DISPLAY_BACKUP_TRADES:
         ...
         if(total<=0)
            MessageBox("Zero results","BackupRestoreOrders");
         else
            if(trr[0].res==RET_OK)
               MessageBox("Order restored","BackupRestoreOrders");
            else
...
void CManagerAPITestDlg::OnUsersCheck()
  {
   BalanceDiff *diff=NULL;
   int          i=-1,login=0,total=1;
   POSITION     pos=m_Results.GetFirstSelectedItemPosition();
   CString      str;
//---
   if(m_display_type!=DISPLAY_BASE_USERS)
      return;
   if(pos==NULL || (i=m_Results.GetNextSelectedItem(pos))<0)
      return;
//---
   login=m_users[i].login;
   diff=m_manager->AdmBalanceCheck(&login,&total);
   str.Format("%d wrong balances",total);
   MessageBox(str,"UsersBalanceCheck");
   if(diff!=NULL)
      m_manager->MemFree(diff);
//---
  }
//+------------------------------------------------------------------+
//|                                                                  |
