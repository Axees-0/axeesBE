...
#define USER_LOGIN   1294890
#define MT_SERVER    "localhost:443"
#define MT_LOGIN     1
#define MT_PASSWORD  "manager"
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
class CManager
...
//--- connect
   if((res=manager->Connect(MT_SERVER))!=RET_OK || (res=manager->Login(MT_LOGIN,MT_PASSWORD))!=RET_OK)
     {
...
//--- open
   ...
   info.orderby=USER_LOGIN;            // order, order by, login
   strcpy(info.symbol,"EURUSD");       // trade symbol
   info.volume =100;                   // trade volume
   info.price  =1.3000;                // trade price
   info.sl     =1.2000;                // stoploss
   info.tp     =1.4000;                // takeprofit
...
//---
   if((res=manager->MarginLevelRequest(USER_LOGIN,&level))!=RET_OK)
     {
