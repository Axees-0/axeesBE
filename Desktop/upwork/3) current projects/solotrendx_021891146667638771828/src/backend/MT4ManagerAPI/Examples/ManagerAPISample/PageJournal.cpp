...
void CPageJournal::OnRequest()
  {
   ...
   CTime  ct;
//---
   mode=GetLogMode(m_Mode.GetCurSel());
   m_From.GetTime(ct);
...
//|                                                                  |
//+------------------------------------------------------------------+
void CPageJournal::OnSelChangeMode()
  {
   int  mode  =GetLogMode(m_Mode.GetCurSel());
   BOOL enable=(mode!=LOG_TYPE_UPDATER && mode!=LOG_TYPE_SENDMAIL && mode!=LOG_TYPE_FAILOVER);
//--- если выставили тип LiveUpdate, то дизейблим дату и режим
...
//|                                                                  |
//+------------------------------------------------------------------+
int CPageJournal::GetLogMode(const int index)
  {
   static int ExtLogTypes[]={LOG_TYPE_STANDARD,LOG_TYPE_LOGINS,LOG_TYPE_TRADES,LOG_TYPE_ERRORS,LOG_TYPE_FULL,LOG_TYPE_UPDATER,LOG_TYPE_SENDMAIL,LOG_TYPE_FAILOVER};
//---
   if(index>=0 && (UINT)index<_countof(ExtLogTypes))
      return(ExtLogTypes[index]);
   return(LOG_TYPE_STANDARD);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
