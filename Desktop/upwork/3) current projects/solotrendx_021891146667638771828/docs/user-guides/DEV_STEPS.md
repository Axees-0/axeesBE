...
> **IMPORTANT DEVELOPMENT APPROACH**: This project follows a Mac-first, Windows-last development approach. Develop and test as much as possible using mocks on macOS, then deploy to Windows for final integration with MT4 only at the end.
...
We've implemented a comprehensive Mock MT4 API that simulates the behavior of the MT4 Manager API without requiring a real MT4 terminal or Windows environment.

#### MT4 Mock API Features:

- **RESTful API Interface**: Exposes the same endpoints as the real MT4 Manager API
- **Trade Execution**: Simulates buy/sell order execution
- **Order Management**: Tracks open orders, allows closing and modifying orders
- **Status Reporting**: Provides server status and order status information

#### Running the MT4 Mock API:
...
The MT4 Mock API will be used by other components (Telegram Bot, Webhook API) for testing and development on macOS. When deploying to Windows, these components will be reconfigured to use the real MT4 Manager API without code changes.
