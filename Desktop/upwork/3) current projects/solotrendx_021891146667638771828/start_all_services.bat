...
start cmd /k "title MT4 REST API && echo Running from %MT4_API_DIR% && call "%VENV_DIR%\Scripts\activate.bat" && set PYTHONPATH=%PROJECT_ROOT% && set USE_MOCK_MODE=true && set PORT=5002 && python "%MT4_API_DIR%\mt4_rest_api_implementation.py""
...
start cmd /k "title Telegram Bot && echo Running from %TELEGRAM_DIR% && call "%VENV_DIR%\Scripts\activate.bat" && set PYTHONPATH=%PROJECT_ROOT% && set MT4_API_URL=http://localhost:5002/api && set FLASK_PORT=5001 && python "%TELEGRAM_DIR%\run.py""
