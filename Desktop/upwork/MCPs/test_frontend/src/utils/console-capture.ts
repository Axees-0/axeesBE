...
import { ConsoleLog } from '../types/workflow.js';
...
export class ConsoleCapture {
   ...
   */
  private processConsoleMessage(msg: ConsoleMessage): ConsoleLog | null {
    try {
      ...
      return {
        ...
        stackTrace: msg.stackTrace()?.map(frame => 
          `  at ${frame.url}:${frame.lineNumber}:${frame.columnNumber}`
        ).join('\n')
      };
    } catch (error) {
   ...
   */
  private shouldCapture(log: ConsoleLog): boolean {
    // Check level filter
   ...
   * Get logs by level
   */
  getLogsByLevel(level: string): ConsoleLog[] {
