import { WorkflowResult, StepResult } from '../types/workflow.js';

export class ReportGenerator {
  /**
   * Generate a report in the specified format
   */
  generate(result: WorkflowResult, format: 'markdown' | 'json' = 'markdown'): string {
    if (format === 'json') {
   ...
   */
  private generateJSON(result: WorkflowResult): string {
   ...
   */
  private generateMarkdown(result: WorkflowResult): string {
   ...
   */
  private generateHeader(result: WorkflowResult): string {
...
${result.error ? `### Error\n\`\`\`\n${result.error}\n\`\`\`` : ''}`;
   ...
   */
  private generateTimeline(result: WorkflowResult): string {
   ...
   */
  private generateConsoleSection(result: WorkflowResult): string {
    const lines: string[] = ['## Console Output'];
   ...
   */
  private generateNetworkSection(result: WorkflowResult): string {
    const lines: string[] = ['## Network Activity'];
   ...
   */
  private generateScreenshotSection(result: WorkflowResult): string {
   ...
   */
  private generatePerformanceSection(result: WorkflowResult): string {
    ...
    const slowestStep = result.steps.reduce((slowest, step, index) => {
      if (!step.duration) return slowest;
      if (!slowest || step.duration > slowest.duration) {
        return { step, index, duration: step.duration };
      }
      return slowest;
    }, null as { step: StepResult; index: number; duration: number } | null);
   ...
   */
  private generateDetailedSteps(result: WorkflowResult): string {
    const lines: string[] = ['## Detailed Step Results'];
    ...
    result.steps.forEach((step, index) => {
      ...
      if (stepDef.selector) lines.push(`- **Selector**: \`${stepDef.selector}\``);
      if (stepDef.value) lines.push(`- **Value**: ${JSON.stringify(stepDef.value)}`);
      lines.push(`- **Status**: ${step.success ? '✅ Success' : '❌ Failed'}`);
      if (step.duration) lines.push(`- **Duration**: ${step.duration}ms`);
