...
## Project: Looking for a Strong Developer Team Specialized in AI Agentic Coding - Web Development
...
**Client Requirements**: 
   - I need help a) improving on the existing website creation pipeline allowing us to create more sites (we enable users to generate sites using 4 different strategies, some of which are more powerful than others) b) adding features to provide functionalities similar to top industry standard tools c) making the site more professional and making minor additions.
   - I want to emphasize that this project should be a lot more of a partnership than simply a job. I'm a university student (at a T5 university) with limited capital to give out. However, I am dedicated, passionate, and want to see this project go to completion. Accordingly, I'm looking for a set of partners who's interests align with mine. Given that we're close to some sort of MVP, I will start leveraging my contacts to get funding when the site is in a suitable form. For now, I'm looking for talented devs who can do high quality work and contribute their original ideas. Feel free to contact me so we can discuss. I'll be responding to proposals that are unique (please do not use AI for proposals).

### Full Job Description
Summary
     I'm currently working an building a tool that creates full stack web apps based on natural language prompts. My tool in its current form is similar to existing ones (v0.dev, bolt.new, devin.ai), and can generate sites on par or better than the existing tools.
