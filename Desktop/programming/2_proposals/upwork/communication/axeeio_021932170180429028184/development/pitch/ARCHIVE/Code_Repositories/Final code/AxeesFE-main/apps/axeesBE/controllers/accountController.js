...
exports.startRegistration = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      message: "Verification code sent via MessageCentral",
...
exports.verifyOtp = async (req, res) => {
  try {
    ...
    if (!phone || !code) {
      return res
        .status(400)
        .json({ message: "Phone number & OTP code are required" });
    }
...
exports.resendOtp = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      message: "New verification code sent successfully via MessageCentral",
...
exports.login = async (req, res) => {
  try {
    ...
    if (!match) {
      return res
        .status(401)
        .json({ message: "Phone or password is incorrect" });
    }
    console.log("match", match, formattedPhone);
    const token = generateToken(user._id);
    ...
    if (!user.isActive) {
      ...
      if (currentStep) {
        return res.status(403).json({
          message: "User is inactive. Please complete your profile info",
          currentStep,
          token,
          userId: user._id,
          user: {
...
exports.verifyResetOtp = async (req, res) => {
  try {
    ...
    if (!user.verificationId) {
      return res.status(400).json({
        message: "No OTP request found. Please initiate password reset.",
      });
    }
...
exports.completePasswordReset = async (req, res) => {
  try {
    ...
    if (!phone || !newPassword) {
      return res.status(400).json({ message: "phone & newPassword required" });
    }
...
exports.uploadMediaPackage = async (req, res) => {
  try {
    ...
    console.log("Found user:", user.userType);
    ...
    console.log("Media package URL:", mediaPackageUrl);
