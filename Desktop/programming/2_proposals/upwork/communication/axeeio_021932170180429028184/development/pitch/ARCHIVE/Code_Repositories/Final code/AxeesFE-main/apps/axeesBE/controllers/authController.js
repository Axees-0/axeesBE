  ...
  try {
    ...
    if (!user) {
      return res.status(404).json({
        message:
          "No pending registration found. Please start the registration process first",
        phone: formattedPhone,
      });
    }
  ...
  try {
    ...
    if (!user.isActive) {
      return res.status(403).json({
        message: "User is inactive. Complete your profile or contact support.",
      });
    }
    ...
    client.messages
      ...
      .then((message) => {
        ...
        res.status(200).json({
          ...
          otpSentAt: user.otpSentAt, // Send this to frontend for timer
        });
      })
      .catch((err) => {
