...
# Mac-compatible Website Uploader
# This script uploads all files from a directory to a server, creating directories as needed
# Works on macOS by avoiding readarray and other Linux-specific features
...
if [ $# -lt 1 ]; then
    ...
    echo -e "${YELLOW}Usage: ./mac_upload.sh <directory_name>${NC}"
    exit 1
fi
...
echo -e "${BLUE}=== Mac-compatible Uploader for $(basename "$DIR") ===${NC}"
...
TEMP_FILE=$(mktemp)
find . -type f -not -path "*/\.*" -not -name "upload_config.txt" -not -name "*.sh" > "$TEMP_FILE"
FILE_COUNT=$(wc -l < "$TEMP_FILE" | tr -d ' ')

echo -e "${GREEN}Found $FILE_COUNT files to upload${NC}"

# Debug output - show first 10 files
echo -e "${BLUE}First 10 files to be uploaded:${NC}"
head -n 10 "$TEMP_FILE"
...
# Use a while loop with redirect, but use a subshell for SCP to preserve variables
while IFS= read -r FILE_PATH; do
    ...
    if [ "$DIR_PATH" != "." ]; then
        ...
        # Create the directory on the server (in a subshell to preserve COUNTER)
        (sshpass -p "$PASSWORD" ssh -p 22 -o StrictHostKeyChecking=no "$SERVER" "mkdir -p $REMOTE_DIR/$DIR_PATH" 2>/dev/null)
    fi
    ...
    # Use a subshell for SCP to preserve variables
