...
if [ $# -lt 1 ]; then
    ...
    echo -e "${YELLOW}Usage: ./ultimate_upload.sh <directory_name>${NC}"
    exit 1
fi
...
echo -e "${BLUE}=== Ultimate Uploader for $(basename "$DIR") ===${NC}"
...
# Find all files directly
readarray -t FILE_LIST < <(find . -type f -not -path "*/\.*" -not -name "upload_config.txt" -not -name "*.sh")
